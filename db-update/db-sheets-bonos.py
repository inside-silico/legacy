...
#Valor Tecnico de bonos CER
url="http://www.bonistas.com/"
sh = gc.open('WebApp_Sheet')
wks = sh.worksheet_by_title("AUX1")
page = requests.get(url, timeout=5)
soup = BeautifulSoup(page.content, 'html.parser')
table = soup.find_all('table')
data = pd.read_html(str(table))[3]
wks.set_dataframe(data,"L1", copy_head = True)

sh = gc.open('Arb_Sheet')
wks = sh.worksheet_by_title("Intra")
wks.clear(start='A2', end='W400')
time.sleep(3)
