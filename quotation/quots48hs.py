...
def example_online():
    ...
    hb.online.subscribe_securities('corporate_bonds','48hs')
    ...
    while now!="17":
        ...
        global ON_dF
        global options_dF    
        sh = gc.open('RV_WebApp')
        wks = sh.worksheet_by_title("Panel")
        wks.set_dataframe(bluechips_dF,(1,1), copy_index=True)
        print("bluechips_dF") 
        time.sleep(3.2)

        sh = gc.open('RV_WebApp')
        wks = sh.worksheet_by_title("Panel")
        wks.set_dataframe(galpones_dF,(27,1), copy_index=True)
        ...
        sh = gc.open('RV_WebApp')
        wks = sh.worksheet_by_title("CEDEAR")
        wks.set_dataframe(CEDEAR_dF,(1,1), copy_index=True)
        print("cedear_df")
        time.sleep(3.2)

        sh = gc.open('WebApp_Sheet')
        wks = sh.worksheet_by_title("Bonos_Coti")
        wks.set_dataframe(bonds_dF,(1,1), copy_index=True)
        print("bonds_df")
        time.sleep(3.7)

        sh = gc.open('WebApp_Sheet')
        wks = sh.worksheet_by_title("ON_Coti")
        wks.set_dataframe(ON_dF,(1,1), copy_index=True)
        time.sleep(3.2)
        print("ON_df")
        time.sleep(3.7)
        now = time.strftime("%H")
